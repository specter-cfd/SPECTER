...
!$    CALL dfftw_plan_with_nthreads(nth)

      CALL dfftw_plan_many_dft_r2c(plan%planr,2,(/n(1),n(2)/),    &
                         kend-ksta+1,plan%rarr,                       &
                         (/n(1),n(2)*(kend-ksta+1)/),1,n(1)*n(2),     &
                         plan%carr,(/n(1)/2+1,n(2)*(kend-ksta+1)/),1, &
                         (n(1)/2+1)*n(2),flags)
      CALL dfftw_plan_many_dft(plan%planc,1,n(3),n(2)*(iend-ista+1), &
...
!$    CALL dfftw_plan_with_nthreads(nth)
      ...
      CALL dfftw_plan_many_dft_c2r(plan%planr,2,(/n(1),n(2)/),    &
...
!
      CALL GTStart(hfft)
      CALL dfftw_execute_dft(plan%planc,out,out)
      CALL GTStop(hfft); ffttime = ffttime + GTGetTime(hfft)
...
!
      CALL GTStart(hfft)
      CALL dfftw_execute_dft(plan%planc,inout,inout)
      CALL GTStop(hfft); ffttime = ffttime + GTGetTime(hfft)
...
! 1D FFT in each node using the FFTW library
         CALL GTStart(hfft)
         CALL dfftw_execute_dft(plan%planc,in,in)
         CALL GTStop(hfft); ffttime = ffttime + GTGetTime(hfft)
