...
!
! 2019 Added i/o support for array subdomains in z. (M. Fontana)
!
...
!-----------------------------------------------------------------
      ...
      INTEGER :: rankend, groupworld, grouptrunc, commtrunc
      INTEGER :: tmp
      ...
      ! Find last "physical" slice and notify last rank to
      ! the other tasks
      tmp = 0
      IF ( n(3) .ge. ksta .AND. n(3) .le. kend) THEN
          tmp = myrank
      ENDIF
   
      CALL MPI_ALLREDUCE(tmp,rankend,1,MPI_INT,MPI_SUM,MPI_COMM_WORLD,ioerr)

      ! Create new communicator with only IO tasks
      CALL MPI_COMM_GROUP(MPI_COMM_WORLD, groupworld, ioerr)
      CALL MPI_GROUP_RANGE_INCL(groupworld,1,(/0,rankend,1/),grouptrunc,ioerr)
      CALL MPI_COMM_CREATE(MPI_COMM_WORLD,grouptrunc,plan%iocomm,ioerr)


      !  Create subarray type for collaborative IO
      IF ( plan%iocomm .ne. MPI_COMM_NULL ) THEN
...
!-----------------------------------------------------------------
      ...
      ! Only tasks in iocomm
      IF ( plan%iocomm .ne. MPI_COMM_NULL) THEN
      CALL GTStart(ihopen)
      IF ( bmangle.EQ.1 ) THEN
         CALL MPI_FILE_OPEN(plan%iocomm,trim(dir) // '/' // fname // '.'&
...
!-----------------------------------------------------------------
      ...
      ELSE
      CALL MPI_FILE_OPEN(plan%iocomm,fname            &
