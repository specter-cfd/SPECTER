...
DO j=1,ny
DO i=1,nx
   R2(i,j,k) = -64*sin(4*x(i))*cos(8*y(j))*sin(6*z(k))
ENDDO
ENDDO
ENDDO
...
DO j=1,ny
DO i=1,nx
   R3(i,j,k) = -36*sin(4*x(i))*cos(8*y(j))*sin(6*z(k))
ENDDO
ENDDO
ENDDO
...
IF ( Cy .gt. 0 ) THEN
   ...
   ! Populate normal derivative at endpoints
   C1(:,1,:)     = C2(:,1,:)     ! d/dy = d/dn
   C1(:,ny-Cy,:) = C2(:,ny-Cy,:)

   ! Load tables and reconstruct
   IF (.NOT. ALLOCATED(planfc%y%neu2)) THEN
       CALL load_neumann_tables(planfc%y,planfc%tdir,2)
   ENDIF
   CALL neumann_reconstruct(planfc,C1,3,2)
   CALL neumann_reconstruct(planfc,C1,4,2)
ELSE
   ! Field and derivatives in (z,ky,kx) domain
...
! Load tables and reconstruct
IF (.NOT. ALLOCATED(planfc%z%neu2)) THEN
   CALL load_neumann_tables(planfc%z,planfc%tdir,2)
ENDIF
CALL neumann_reconstruct(planfc,C1,5,2)
CALL neumann_reconstruct(planfc,C1,6,2)
...
! -------------------------------------------
! Print 2nd derivative errors in real space
! -------------------------------------------
! Exact derivative in y direction
DO k=ksta,pkend
DO j=1,ny
