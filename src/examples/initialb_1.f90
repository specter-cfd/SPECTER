         ...
         DO j = 2,ny/2+1
            IF ((kk2(1,j,1).le.kup**2).and.(kk2(1,j,1).ge.kdn**2)) THEN
               ...
               DO k = 1, nz-Cz
                  ...
                  rmq = COS(rm1)/sqrt(kk2(1,j,1)+rm1**2)**3
                  rms = SIN(rm2)/sqrt(kk2(1,j,1)+rm2**2)**3

                  ! Even contribution first, then odd contribution
                  C1(k,j,1) = rmq*(COS(rmp)+im*SIN(rmp))*(&
                        COSH(rm1*(z(k)/Lz-0.5))/COSH(0.5*rm1) -&
                        COS(rm1*(z(k)/Lz-0.5))/COS(0.5*rm1))
                  C1(k,j,1) = C1(k,j,1) +  rms*(COS(rmp)+im*SIN(rmp))*(&
...
!$omp parallel do if (iend-ista-1.lt.nth) private (k,rmp,rmq,rms)
            DO j = 1,ny
               ...
               DO k = 1, nz-Cz
                  ! Amplitude of even and odd contributions
                  rmq = COS(rm1)/sqrt(kk2(1,j,i)+rm1**2)**3
                  rms = SIN(rm2)/sqrt(kk2(1,j,i)+rm2**2)**3

                  ! Even contribution first, then odd contribution
                  C1(k,j,i) = rmq*(COS(rmp)+im*SIN(rmp))*(&
                        COSH(rm1*(z(k)/Lz-0.5))/COSH(0.5*rm1) -&
                        COS(rm1*(z(k)/Lz-0.5))/COS(0.5*rm1))
                  C1(k,j,i) = C1(k,j,i) +  rms*(COS(rmp)+im*SIN(rmp))*(&
                        SINH(rm2*(z(k)/Lz-0.5))/SINH(0.5*rm2) -&
                        SIN(rm2*(z(k)/Lz-0.5))/SIN(0.5*rm2))
               ENDDO
               ENDIF
