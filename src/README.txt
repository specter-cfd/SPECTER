...
See the file 'SOLVERS.txt' for a list of the implemented solvers.
...
tstep and cstep: number of time steps done between exporting global
      quantities (cstep), and binary files (tstep).
...
'examples/parameter.txt' and 'specter.fpp' for more details.
...
"README_output.info", and placed in the same directory as the
